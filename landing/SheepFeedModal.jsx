...
import HeroLambModal from './HeroLambModal';
...
const getProductsData = (lang) => [
  {
    key: 'hero-lamb',
    name: 'HERO® Lamb Milk Replacer',
    color: COLOR,
    glow: 'rgba(236,72,153,0.5)',
    desc: 'Dedicated formulation for lambs — fortified with prebiotics & probiotics, fat & water-soluble vitamins, trace minerals. Contains 24% protein, 22% fat. Complete feeding chart included.',
  },
];

export default function SheepFeedModal({ open, onClose }) {
  const { lang } = useLang();
  const PRODUCTS = getProductsData(lang);
  const [heroLambOpen, setHeroLambOpen] = useState(false);

  return (
    <>
      <HeroLambModal open={heroLambOpen} onClose={() => setHeroLambOpen(false)} />
      <AnimatePresence>
        {open && (
          <motion.div
          ...
          >
            <motion.div
              ...
              style={{
                background: 'linear-gradient(145deg, rgba(12,4,8,0.97) 0%, rgba(8,2,6,0.99) 100%)',
              ...
              <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-mono text-[9px] tracking-[3px] mt-0.5" style={{ color: `${COLOR}88` }}>SHEEP · OVINE NUTRITION</div>
                  </div>
                </div>
              ...
              <div className="p-6 space-y-3">
                {PRODUCTS.map((product, i) => (
                  <motion.div
                    ...
                    onClick={() => { if (product.key === 'hero-lamb') setHeroLambOpen(true); }}
                  >
