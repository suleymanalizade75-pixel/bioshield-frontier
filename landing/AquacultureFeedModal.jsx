...
import VitaquamixModal from './VitaquamixModal';
import GrowAquaModal from './GrowAquaModal';
...
const getFeedCategoriesData = (lang) => [
  {
    ...
    desc: 'High-protein amino blend — improves FCR by 18% in tilapia & salmon.',
    products: [
      { name: 'Toxi-Guard', modal: 'toxi-guard' },
    ],
  },
  {
    key: 'pigmentation',
    label: 'Pigmentation & Quality',
    color: '#1A82C0',
    glow: 'rgba(26,130,192,0.5)',
    desc: 'Astaxanthin & spirulina — enhances growth rate and shell pigmentation.',
    products: [
      { name: 'ShrimpVital', desc: 'Premium colorant blend for superior shell and flesh pigmentation.' },
      { name: 'Vitaquamix', modal: 'vitaquamix' },
    ],
  },
  {
    key: 'gut-health',
...
export default function AquacultureFeedModal({ open, onClose }) {
  const { lang } = useLang();
  const FEED_CATEGORIES = getFeedCategoriesData(lang);
  const [hovered, setHovered] = useState(null);
  const [locked, setLocked] = useState(null);
  const [toxiGuardOpen, setToxiGuardOpen] = useState(false);
  const [vitaquamixOpen, setVitaquamixOpen] = useState(false);
  const [growAquaOpen, setGrowAquaOpen] = useState(false);
  ...
  const handleProductClick = (modalKey) => {
    ...
    if (modalKey === 'vitaquamix') setVitaquamixOpen(true);
    if (modalKey === 'grow-aqua') setGrowAquaOpen(true);
  };
  ...
  return (
    ...
    <VitaquamixModal open={vitaquamixOpen} onClose={() => setVitaquamixOpen(false)} />
    <GrowAquaModal open={growAquaOpen} onClose={() => setGrowAquaOpen(false)} />
    <AnimatePresence>
      {open && (
        <motion.div
          initial={{ opacity: 0 }}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-mono text-[9px] tracking-[3px] mt-0.5" style={{ color: 'rgba(26,130,192,0.55)' }}>
                    NUTRITION SOLUTIONS FOR FISH & SHRIMP
                  </div>
                </div>
              </div>
              <button
