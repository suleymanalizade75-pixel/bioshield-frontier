...
const getModalData = (lang) => ({
  tags: lang === 'EN' ? ['TanniFit Plus', 'Liquid Pump', 'Early Care'] : lang === 'AZ' ? ['TanniFit Plus', 'Maye Nasos', 'Erkən Qayğı'] : ['TanniFit Plus', 'Жидкий Насос', 'Ранний Уход'],
  features: lang === 'EN' ? 'TanniFit Plus is a dietary complementary feed with botanical ingredients in order to stabilize water and electrolyte balance to support a physiological digestion in the early life of suckling piglets, lambs and goat kids. The tannin-rich formula has astringent and anti-inflammatory effects on the gut mucosa. Essential oils of Thyme and Oregano are well-known for their antimicrobial effects on Gram+ and Gram- bacteria, anti-fungal effects, as well as their potential to reduce oxidative stress. The combination of these botanical ingredients might stimulate the appetite during suckling phase.' : lang === 'AZ' ? 'TanniFit Plus əmzikli donuz balalarının, quzuların və keçi cavanlarının erkən həyatında fizioloji həzmi dəstəkləmək üçün su və elektrolit balansını sabitləşdirmək məqsədilə botanik inqrediyentlər olan dietik tamamlayıcı yemdir. Tannin-zəngin formula bağırsaq mukozasına büzücü və iltihab əleyhinə təsirlər göstərir. Kəklikotu və oregano əssensial yağları antimikrob, antifungal təsirləri ilə tanınır. Bu botanik inqrediyentlərin birləşməsi əmzikləmə fazasında iştahanı stimullaşdıra bilər.' : 'TanniFit Plus — диетический дополнительный корм с растительными ингредиентами для стабилизации водно-электролитного баланса и поддержки физиологического пищеварения в раннем возрасте. Богатая танинами формула оказывает вяжущее и противовоспалительное действие. Сочетание растительных ингредиентов стимулирует аппетит в период кормления.',
...
export default function EarlyCareModal({ open, onClose }) {
  const { lang } = useLang();
          ...
          <motion.div initial={{ scale: 0.88, opacity: 0, y: 40 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.94, opacity: 0, y: 20 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(145deg, rgba(8,28,12,0.97) 0%, rgba(5,18,8,0.99) 100%)', border: '1px solid rgba(34,211,102,0.3)', boxShadow: '0 0 60px rgba(34,211,102,0.12), 0 40px 80px rgba(0,0,0,0.7)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="rounded-xl p-5" style={{ background: 'rgba(34,211,102,0.06)', border: '1px solid rgba(34,211,102,0.2)' }}>
                ...
                <div className="space-y-2">{data.benefits.map(b => (<div key={b} className="flex items-start gap-2"><CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style={{ color: '#22c55e' }} /><span className="font-inter text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.7)' }}>{b}</span></div>))}</div>
              </div>
