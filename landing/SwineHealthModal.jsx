...
import {
  X, Heart, Activity, Shield, Droplets,
  FlaskConical, Zap, Wind, Sparkles, ChevronRight, Thermometer
} from 'lucide-react';
import { useLang } from '@/lib/i18n';
import TanniFitPlusLiquidModal from './TanniFitPlusLiquidModal';
...
const getHealthCategories = (lang) => [
  {
    ...
    desc: lang === 'EN' ? 'Neonatal support, colostrum enhancement & optimal piglet development protocols.' : lang === 'AZ' ? 'Yeni doğulmuş dəstəyi, kolostrum güçləndirmə və optimal piqlet inkişaf protokolları.' : 'Поддержка новорождённых, улучшение молозива и оптимальные протоколы развития поросят.',
    products: ['TanniFit Plus Liquid (Early Care)'],
  },
  {
    key: 'gut',
...
export default function SwineHealthModal({ open, onClose }) {
  const { lang } = useLang();
  ...
  const handleProductClick = (productName) => {
    if (productName === 'TanniFit Plus Liquid') setTanniFitOpen(true);
    ...
    else if (productName === 'TanniFit Plus Liquid (Early Care)') setEarlierCareOpen(true);
  ...
  return (
    <>
    <TanniFitPlusLiquidModal open={tanniFitOpen} onClose={() => setTanniFitOpen(false)} />
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                >
                  <Shield className="w-4 h-4" style={{ color: '#587012', filter: 'drop-shadow(0 0 5px #587012)' }} />
                </div>
                <div>
                  <div className="font-orbitron text-sm font-bold text-white tracking-[3px]">
                    {lang === 'EN' ? 'SWINE · HEALTH' : lang === 'AZ' ? 'DONUZ · SAĞLAMLIQ' : 'СВИНЬИ · ЗДОРОВЬЕ'}
                  </div>
                  <div className="font-mono text-[9px] tracking-[3px] mt-0.5" style={{ color: 'rgba(136,184,74,0.55)' }}>
                    {lang === 'EN' ? '6 SOLUTION CATEGORIES' : lang === 'AZ' ? '6 HƏLL KATEQORİYASI' : '6 КАТЕГОРИЙ РЕШЕНИЙ'}
                  </div>
                </div>
              </div>
              <button
