...
const getHealthCategories = (lang) => [
  {
    key: 'immune',
    label: lang === 'EN' ? 'Immune Support' : lang === 'AZ' ? 'İmmunitet Dəstəyi' : 'Поддержка Иммунитета',
    color: '#1A82C0',
    glow: 'rgba(26,130,192,0.5)',
    products: [
      { name: 'AquaImmune Pro', desc: lang === 'EN' ? 'Beta-glucans & Vit C — reduces mortality from bacterial gill disease.' : lang === 'AZ' ? 'Beta-glükanlar və Vit C — bakterial sağ ölümlülüyünü azaldır.' : 'Бета-глюканы и витамин С — снижает смертность от бактериального жаберного заболевания.' },
    ],
  },
  {
    key: 'pathogen',
    label: lang === 'EN' ? 'Pathogen Control' : lang === 'AZ' ? 'Patogen Nəzarəti' : 'Контроль Патогена',
    color: '#1A82C0',
    glow: 'rgba(26,130,192,0.5)',
    products: [
      { name: 'PondGuard AV', desc: lang === 'EN' ? 'Organic acids + probiotics — controls Vibrio & Aeromonas loads.' : lang === 'AZ' ? 'Üzvi turşular + probiotiklər — Vibrio və Aeromonas yüklərini nəzarət edir.' : 'Органические кислоты + пробиотики — контролирует нагрузку Vibrio и Aeromonas.' },
    ],
  },
];
...
export default function AquacultureHealthModal({ open, onClose }) {
  const { lang } = useLang();
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-orbitron text-sm font-bold text-white tracking-[3px]">
                    {lang === 'EN' ? 'AQUACULTURE · HEALTH' : lang === 'AZ' ? 'SUAQUA TƏSƏRRÜFATI · SAĞLAMLIQ' : 'АКВАКУЛЬТУРА · ЗДОРОВЬЕ'}
                  </div>
                  <div className="font-mono text-[9px] tracking-[3px] mt-0.5" style={{ color: 'rgba(26,130,192,0.55)' }}>
                    {lang === 'EN' ? 'HEALTH SOLUTIONS FOR FISH & SHRIMP' : lang === 'AZ' ? 'BALIQ VƏ XƯS ÜÇÜN SAĞLAMLIQ HƏLLƏRI' : 'РЕШЕНИЯ ДЛЯ ЗДОРОВЬЯ РЫБ И КРЕВЕТОК'}
                  </div>
                </div>
              </div>
              <button
                ...
                {!hovered && !locked ? (
                  ...
                  >
                    <span className="font-mono text-[10px] tracking-[4px]" style={{ color: 'rgba(255,255,255,0.2)' }}>
                      {lang === 'EN' ? '↑ SELECT A CATEGORY TO VIEW PRODUCTS' : lang === 'AZ' ? '↑ MƏHSULLARI GÖRMƏK ÜÇÜN KATEQORİYA SEÇİN' : '↑ ВЫБЕРИТЕ КАТЕГОРИЮ ДЛЯ ПРОСМОТРА ПРОДУКТОВ'}
                    </span>
                  </motion.div>
                ) : (
                  <motion.div
